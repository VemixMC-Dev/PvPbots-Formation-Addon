 ...
 *  SQUARE          – Bots evenly distributed around a square perimeter, face inward
 *  LINE            – Line formation; radius controls row count:
 *                      radius <= 0 or -1  → single depth line pointing toward player
 *                      radius == 1        → single horizontal line (all visible side-by-side)
 *                      radius > 1         → split bots into that many horizontal rows
 *
...
public class FormationManager {
    ...
    public static List<double[]> computeSlots(Vec3d anchorPos,
        ...
        switch (formation) {
            ...
            case SQUARE:
                return computeSquare(anchorPos, anchorYaw, count, resolveRadius(radius, count, 3.0));
            case LINE:
                return computeLine(anchorPos, anchorYaw, count, radius);
            case GRID:
            default:
    ...
    // =========================================================================
    // Square  (bots evenly spaced around a square perimeter, face inward)
    // =========================================================================

    private static List<double[]> computeSquare(Vec3d anchorPos, float anchorYaw,
                                                 int count, double radius) {
        List<double[]> slots = new ArrayList<>(count);
        double ax = anchorPos.x, ay = anchorPos.y, az = anchorPos.z;
        double baseRad = Math.toRadians(anchorYaw);

        // The four corner angles of a square rotated to match anchorYaw.
        // We walk around the perimeter by distributing bots proportionally
        // to each side's length (all sides equal, so perEdge is uniform).
        double half = radius;                          // half-side length
        // Corners in Minecraft (−sin/+cos) world space, rotated by anchorYaw:
        //   fwd  = (−sin, cos),  right = (cos, sin)
        double fwdX = -Math.sin(baseRad), fwdZ = Math.cos(baseRad);
        double rgtX =  Math.cos(baseRad), rgtZ = Math.sin(baseRad);

        // Four corners: front-left, front-right, back-right, back-left
        double[][] corners = {
            {ax + fwdX * half - rgtX * half, az + fwdZ * half - rgtZ * half},
            {ax + fwdX * half + rgtX * half, az + fwdZ * half + rgtZ * half},
            {ax - fwdX * half + rgtX * half, az - fwdZ * half + rgtZ * half},
            {ax - fwdX * half - rgtX * half, az - fwdZ * half - rgtZ * half}
        };

        // Distribute bots evenly across the 4 sides (extras go to front sides first).
        int perSide = count / 4;
        int extra   = count % 4;

        for (int side = 0; side < 4; side++) {
            int botsOnSide = perSide + (side < extra ? 1 : 0);
            double[] c0 = corners[side];
            double[] c1 = corners[(side + 1) % 4];
            for (int i = 0; i < botsOnSide; i++) {
                // Place bots evenly along the edge, including the start corner
                // but not the end (to avoid double-placing corners).
                double t  = botsOnSide == 1 ? 0.5 : (double) i / botsOnSide;
                double bx = c0[0] + t * (c1[0] - c0[0]);
                double bz = c0[1] + t * (c1[1] - c0[1]);
                float  yaw = yawFacingToward(bx, bz, ax, az);
                slots.add(new double[]{bx, ay, bz, yaw});
            }
        }
        return slots;
    ...
    // =========================================================================
    // Line  (rows of bots; radius encodes row count)
    //
    //   radius <= 0 or -1  → single depth line pointing toward player (1 column)
    //   radius == 1        → single horizontal row (all bots side-by-side)
    //   radius > 1         → split into that many horizontal rows
    // =========================================================================

    private static List<double[]> computeLine(Vec3d anchorPos, float anchorYaw,
                                              int count, double radiusRaw) {
        List<double[]> slots = new ArrayList<>(count);
        double ax = anchorPos.x, ay = anchorPos.y, az = anchorPos.z;
        double rad = Math.toRadians(anchorYaw);
        double fwdX = -Math.sin(rad), fwdZ =  Math.cos(rad);
        ...
        final double COL_GAP = 1.5;  // spacing between bots side-by-side
        final double ROW_GAP = 1.5;  // spacing between rows (depth)
        final double START   = 1.5;  // distance in front of anchor for first row

        // Depth line: single column pointing toward (in front of) the player.
        if (radiusRaw <= 0) {
            for (int i = 0; i < count; i++) {
                double dist = START + i * ROW_GAP;
                slots.add(new double[]{
                    ax + fwdX * dist,
                    ay,
                    az + fwdZ * dist,
                    anchorYaw
        ...
        // Horizontal rows (radius == 1 → 1 row, radius > 1 → multiple rows).
        int numRows = Math.max(1, (int) Math.round(radiusRaw));
        int botsPerRow = (int) Math.ceil((double) count / numRows);

        int remaining = count;
        for (int row = 0; row < numRows && remaining > 0; row++) {
            int botsInRow = Math.min(botsPerRow, remaining);
            remaining -= botsInRow;
            double rowDist = START + row * ROW_GAP;
            for (int col = 0; col < botsInRow; col++) {
                double colOffset = (col - (botsInRow - 1) / 2.0) * COL_GAP;
                slots.add(new double[]{
                    ax + fwdX * rowDist + rgtX * colOffset,
                    ay,
                    az + fwdZ * rowDist + rgtZ * colOffset,
                    anchorYaw
                });
            }
        }
